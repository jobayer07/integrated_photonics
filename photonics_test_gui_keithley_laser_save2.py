"""
Created on Wed Aug 05 2021
Graphical User Interface (GUI) for optical and electrical testing of silicon photonics
...
layout = [  #[sg.Text("GPIB"), sg.InputCombo([20, 21, 22, 23, 24, 25, 26], default_value='24', size=(4,1), key='gpib_number'), sg.Button('Initiate', border_width=5, size=(4,1), button_color='orangered', mouseover_colors=('yellow','black')), sg.Text("Status"), sg.Output(background_color='white', size=(50,1))],
            ...
            [sg.Text(" ")],
            [sg.Text("Keithley Settings:"), sg.Text("Start Voltage (v)"), sg.Input(background_color='white', size=(5,1)), sg.Text("Stop Voltage (v)"), sg.Input(background_color='white', size=(5,1)), sg.Text("Step (V)"), sg.Input(background_color='white', size=(5,1)), sg.Button('Voltage Sweep', button_color='royalblue')],
            ...
            [sg.Text("                          "), sg.Graph((200, 200), (-10, -10), (10, 10), background_color='white', border_width=0, key='iv_graph')],
            #[sg.Text(" ")],
            [sg.Text("Laser Settings:"), sg.Text("Align Wavelength (nm)"), sg.Input(background_color='white', size=(5,1)), sg.Text("Laser Power (dBm)"), sg.Input(background_color='white', size=(5,1)), sg.Button('Laser Sweep', button_color='royalblue'), sg.Button('Save Data', button_color='royalblue')],
...
def draw_power_vs_wavelength_figure(canvas, x, y):
    ...
    #data_dir='C:\lsip-measurement-pycontrol\Jet2\test'
    data_with_wl=np.zeros((power.shape[0],2))
    data_with_wl[:,0]=wavelength
    data_with_wl[:,1]=power
    #data_with_wl[:,1]=dbdata_b
    np.savetxt('data.csv',data_with_wl,delimiter=',')  
...
if __name__=="__main__":
    ...
    while True:
        ...
        if event=='Voltage Sweep':
            ...
            #iv_graph.erase()
            try:
                V=np.arange(float(values[1]), float(values[2]), float(values[3]))
                len(V)
                I=np.zeros(len(V))
                m=0
                ...
                print('Voltage sweep done', 'V', V, ' I=', I)
            except:
        ...
        if event=='Current Sweep':
            ...
            #iv_graph.erase()
            try:
                V,I=kthl.current_sweep(float(values[4]), float(values[5]), float(values[6]))   #kthl.current_sweep(start, stop, step)
                #draw_iv_graph()
                draw_figure(window['iv_graph'].TKCanvas, V, I)
                print('Result: Current sweep is done')
            except:
                print('Code did not run successfully')

        if event=='Laser Sweep':
            #power_graph.erase()
            try:
                wavelength,power=laser_sweep(float(values[9]), float(values[10]), float(values[11]))
                ...
                d={}
                for x in v: # for every element x in v
                    wavelength,power=laser_sweep(float(values[9]), float(values[10]), float(values[11]))
                    d ['Wavelength (nm)']=wavelength 
                    d['Power(dBm) @ ' + str (x) +'V']= power
                print('Sweep done')
            except:
                print('Code did not run successfully')
        if event=='Save Data':
            try:
                ...
                print('Data Saving was not successful') 
